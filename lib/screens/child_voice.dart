...
class _VoiceFlowState extends State<VoiceFlow> with SingleTickerProviderStateMixin {
  ...
  Future<void> _initVoice() async {
    ...
    if (!_voiceReady) {
      _voiceReady = true;
      if (mounted) {
        await VoiceService.instance.speak("I am ready. Tell me what you want me to do.");
      }
    }
              ...
              child: Container(
                ...
                child: const Center(
                  child: Column(
                    ...
                    children: [
                      Icon(Icons.search_rounded, color: Colors.white, size: 32),
                      SizedBox(height: 6),
                      Text(
                        "OBJECT",
                        style: TextStyle(
              ...
              child: Container(
                ...
                child: const Center(
                  child: Column(
                    mainAxisAlignment: MainAxisAlignment.center,
                    children: [
                      Icon(Icons.remove_red_eye_rounded, color: Colors.white, size: 32),
                      SizedBox(height: 6),
                      Text(
                        "SCENE",
                        style: TextStyle(
        ...
        children: [
          ...
          Row(children: [
            ...
            IconButton(
              ...
              icon: const Icon(Icons.people_alt_outlined, color: Colors.white70, size: 26),
              tooltip: "Unknowns",
            ),
            IconButton(
              onPressed: () => Navigator.pushNamed(context, "/child-camera"),
              icon: const Icon(Icons.camera_alt, color: Colors.lightBlueAccent, size: 26),
              tooltip: "Phone Camera",
            ),
            IconButton(
              onPressed: () => Navigator.pushNamed(context, "/camera-ai"),
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          Expanded(
            child: Text(
              currentLiveText.isEmpty ? "Listening..." : currentLiveText,
        ...
        child: Container(
          ...
          child: Row(
            ...
            children: [
              ...
              Text(
                _pushToTalkActive ? "Listening..." : "Hold to Speak",
  ...
  Future<void> _handleObjectCommand({bool speakResponse = false}) async {
    if (speakResponse) {
      ...
      final msg = language == 'ar' ? "حسناً، سأفتح التعرف على الأشياء." : "Opening Object Detection.";
      _addMessage(msg, false);
    }
    if (mounted) {
      Navigator.pushNamed(context, "/camera-ai", arguments: {'mode': 'object'});
    }
  }
  ...
  Future<void> _handleSceneCommand({bool speakResponse = false}) async {
    if (speakResponse) {
      final prefs = await SharedPreferences.getInstance();
      final language = prefs.getString('language') ?? 'en';
      final msg = language == 'ar' ? "حسناً، سأفتح وصف المشهد." : "Opening Scene Description.";
      _addMessage(msg, false);
    }
    if (mounted) {
      Navigator.pushNamed(context, "/camera-ai", arguments: {'mode': 'scene'});
    }
  }
  ...
  Future<void> _sendAlert() async {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text(ok ? "Alert sent to parents." : "Failed to send alert."),
