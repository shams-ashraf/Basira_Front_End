...
class _UnknownScreenState extends State<UnknownScreen> {
  ...
  Future<void> load() async {
    ...
    try {
      final localUnknowns = await FaceRecognitionService.instance.getUnknownPersons();
  ...
  Future<void> save(String id, String imagePath, String source) async {
    ...
    if (name.isEmpty) {
      _snack("Please enter a name for this person.", isError: true);
      return;
    }
    ...
    try {
      ...
      _snack("Saved $name successfully.");
    } catch (e) {
      _snack("Failed to save unknown person: $e", isError: true);
    } finally {
      if (mounted) {
        ...
        child: data.isEmpty
            ? ListView(
                children: [
                  ...
                  const Center(child: Text("لا يوجد أشخاص مجهولون")),
                  const SizedBox(height: 8),
                  const Center(child: Text("الصور التي يلتقطها الطفل ستظهر هنا للأهل حسب اليوزر المرتبط")),
                ],
              )
            : ListView.builder(
